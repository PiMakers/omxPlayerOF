...
#include "OmxPubHubPlayer.h"
    ...
    OmxPubHubPlayer::OmxPubHubPlayer (){
    ofLogNotice ("OPHP Initialised!!!!!");
    }
    OmxPubHubPlayer::~OmxPubHubPlayer (){
    exit();
    ofLogError(__func__) << "#####################";
...
/*
void OmxPubHubPlayer::sig_handler(int s) {
  if (s==SIGINT && !g_abort)
  {
  ...
  ofLogNotice ("HHHHHHHUUUUUUUUUUUUU");
  abort();
}
*/
void OmxPubHubPlayer::print_usage() {
  printf(
  #include "help.h"
  );
}

void OmxPubHubPlayer::print_keybindings() {
  printf(
  #include "keys.h"
  );
}

void OmxPubHubPlayer::print_version() {
...
void OmxPubHubPlayer::PrintSubtitleInfo() {
...
void OmxPubHubPlayer::SetSpeed(int iSpeed) {
  if(!m_av_clock)
    return;
...
float OmxPubHubPlayer::get_display_aspect_ratio(HDMI_ASPECT_T aspect) {
...
float OmxPubHubPlayer::get_display_aspect_ratio(SDTV_ASPECT_T aspect) {
  float display_aspect;
  switch (aspect) {
...
void OmxPubHubPlayer::FlushStreams(double pts) {
  m_av_clock->OMXStop();
  m_av_clock->OMXPause();
...
void OmxPubHubPlayer::CallbackTvServiceCallback(void *userdata, uint32_t reason, uint32_t param1, uint32_t param2) {
...
void OmxPubHubPlayer::SetVideoMode(int width, int height, int fpsrate, int fpsscale, FORMAT_3D_T is3d) {
  int32_t num_modes = 0;
  int i;
...
bool OmxPubHubPlayer::Exists(const std::string& path) {
  struct stat buf;
  auto error = stat(path.c_str(), &buf);
  return !error || errno != ENOENT;
}

bool OmxPubHubPlayer::IsURL(const std::string& str) {
...
bool OmxPubHubPlayer::IsPipe(const std::string& str) {
  if (str.compare(0, 5, "pipe:") == 0)
    return true;
  return false;
}

int OmxPubHubPlayer::get_mem_gpu(void) {
   char response[80] = "";
   int gpu_mem = 0;
...
void OmxPubHubPlayer::blank_background(uint32_t rgba) {
...
int OmxPubHubPlayer::omxPlayerCli (int argc, char** argv) {
  ...
  */
  ...
  /* int */control_err = m_omxcontrol.init( ///*H Declaration moved to root because of threadedfunc.
    m_av_clock,
    &m_player_audio,
    &m_player_subtitles,
    &m_omx_reader,
...
void OmxPubHubPlayer::threadedFunction() {
  ofLogNotice(__func__) << "player1" << m_send_eos;
  ...
  */
    ...
    ///*H cout << "NOW =  " << now << endl;
    bool update = false;
    if (m_last_check_time == 0.0 || m_last_check_time + DVD_MSEC_TO_TIME(200) <= now) ///*H
    {
      ///*H cout << "DVD_MSEC_TO_TIME(200) =  " << DVD_MSEC_TO_TIME(200) << endl;
      update = true;
  ...
  cout << "EXIT_RETURNED" << exit();
}

int OmxPubHubPlayer::exit() {
  if (m_stats)
    printf("\n");
