...
puBHubPlayer::puBHubPlayer() {
  m_send_eos  = false;
  m_stop      = false;
  ofLogNotice (__func__) << "initialised!!!! ";
  ofLogNotice (__func__) << "m_loop_from!!!! " << m_loop_from;
  ofLogNotice (__func__) << "m_gen_log!!!! " << m_gen_log;
  ofLogNotice (__func__) << "last_seek_pos!!!! " << last_seek_pos;
  ofLogNotice (__func__) << "m_incr!!!! " << m_incr;
  ofLogNotice (__func__) << "startpts!!!! " << startpts;
  ofLogNotice (__func__) << "m_audio_extension!!!! " << m_audio_extension;
  ofLogNotice (__func__) << "m_stats!!!! " << m_stats;
  m_gen_log = false;
  last_seek_pos = 0;
  m_incr = 0;
  startpts = 0;
...
///volatile sig_atomic_t puBHubPlayer::g_abort           = false;

/// Keyboard*         puBHubPlayer::m_keyboard;
/*
void puBHubPlayer::sig_handler(int s) {
  if (s==SIGINT && !g_abort)
  {
...
*/
void puBHubPlayer::print_usage() {
  printf(
  #include "help.h"
  );
}

void puBHubPlayer::print_keybindings() {
  printf(
 #include "keys.h"
  );
}

void puBHubPlayer::print_version() {
...
void puBHubPlayer::PrintSubtitleInfo() {
...
void puBHubPlayer::SetSpeed(int iSpeed) {
  if(!m_av_clock)
    return;
...
float puBHubPlayer::get_display_aspect_ratio(HDMI_ASPECT_T aspect) {
...
float puBHubPlayer::get_display_aspect_ratio(SDTV_ASPECT_T aspect) {
  float display_aspect;
  switch (aspect) {
...
void puBHubPlayer::FlushStreams(double pts) {
  m_av_clock->OMXStop();
  m_av_clock->OMXPause();
...
void puBHubPlayer::CallbackTvServiceCallback(void *userdata, uint32_t reason, uint32_t param1, uint32_t param2) {
...
void puBHubPlayer::SetVideoMode(int width, int height, int fpsrate, int fpsscale, FORMAT_3D_T is3d) {
  int32_t num_modes = 0;
  int i;
...
bool puBHubPlayer::Exists(const std::string& path) {
  struct stat buf;
  auto error = stat(path.c_str(), &buf);
  return !error || errno != ENOENT;
}

bool puBHubPlayer::IsURL(const std::string& str) {
...
bool puBHubPlayer::IsPipe(const std::string& str) {
  if (str.compare(0, 5, "pipe:") == 0)
    return true;
  return false;
}

int puBHubPlayer::get_mem_gpu(void) {
   char response[80] = "";
   int gpu_mem = 0;
...
void puBHubPlayer::blank_background(uint32_t rgba) {
...
void puBHubPlayer::threadedFunction() {
  ...
  while(!m_stop) {
    ...
    */
    ...
    if (!sentStarted)
    {
      CLog::Log(LOGDEBUG, "CpuBHubPlayer::HandleMessages - player started RESET");
...
/*
int puBHubPlayer::omxPlayerCli2 (char* str) {
...
*/
int puBHubPlayer::omxPlayerCli (int argc, char** argv) {
  /*
  ...
  */
  ...
  change_file();
  return 0;
} /// OMXPLayer init ended

void puBHubPlayer::change_file () {
...
int puBHubPlayer::do_exit() {
  ofLogNotice(__func__) << "HOPPPPP!!!!!!";
  if (m_stats)
    printf("\n");
  ...
  if (m_send_eos) {
        ofLogNotice(__func__) << "m_send_eos TRUE!!!!!!!";
    return 0;
    }
  // exit status OMXPlayer defined value on user quit
  if (m_stop) {
        ofLogNotice(__func__) << "m_stop TRUE!!!!!!!";
    return 3;
    }
  // exit status failure on other cases
    ofLogNotice(__func__) << "return EXIT_FAILURE";
///    waitForThread(true);
  ofLogError(__func__) << "EXIT_FAILURE!!!";   sleep(10);
  return EXIT_FAILURE;
}
